...
const PatientRecord = ({ isPatient }) => {
  ...
  const [isDiagonsticTestEditing, setIsDiagonsticTestEditing] =
  ...
  const removeDiagnosticTest = (index: number) => {
    if (currentTicket) {
      const newDiagnostics = currentTicket.prescription[0].diagnostics.filter(
        (_, i) => i !== index
      );
      setCurrentTicket({
        ...currentTicket,
  ...
  return (
    <>
              ...
              {isEditing ? (
                <Stack display="flex" flexDirection="row">
                  ...
                  <button
                    ...
                    onClick={(event) =>
                      handleAdmissionSubmit(event, admissionType)
                    }
                    style={{
                  ...
                  <Stack flexWrap={'wrap'} flexDirection="row" gap={'14px'}>
                    {[
                      ...
                      <button
                        ...
                        style={{
                          backgroundColor:
                            admissionType == item ? '#DAE8FF' : '#F6F7F9',
                          fontSize: '12px'
                        }}
                        onClick={() => {
                          setAdmissionType(item);
      ...
      {currentTicket?.prescription?.[0]?.diagnostics?.length > 0 &&
      currentTicket?.prescription?.[0]?.diagnostics[0] !== null ? (
        <Stack className="gray-border">{/* Borders */}</Stack>
      ) : (
        <></>
      )}
                ...
                {isDiagonsticTestEditing ? (
                  ...
                  !isAuditor && (
                    <Stack
                      ...
                      className="edit-icon"
                      sx={{
                        marginLeft: isDiagonsticTestEditing ? '10px' : '0'
                      }}
      ...
      <Box className="Patient-records Pharmacy">
        ...
        <Box
          className="Patient-records-Head"
          display={'flex'}
        ...
        >
          {(currentTicket?.prescription?.[0]?.medicines?.length ?? 0) > 0 ? (
            <>
              ...
              <Box className="record-tag pharmacy-tag" marginTop={'5px'}>
            ...
            <>
              <Box
                ...
                sx={{
                  width:
                    currentTicket?.prescription[0]?.isPharmacy == 'Not Advised'
                      ? '102px;'
                      : 'width: 132px;'
                }}
              >
