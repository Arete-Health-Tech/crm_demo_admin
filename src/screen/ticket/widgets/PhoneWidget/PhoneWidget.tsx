...
const PhoneWidget = () => {
  ...
  const hasRecording = currentTicket?.phoneData?.some(
    (item) => item.recording !== null && durations[item.recording] > 0
  );
          ...
          className={
            !isAuditor
              ? localStorage.getItem('ticketType') !== 'Follow-Up'
                ? styles.phoneBox
                : styles.phoneBoxFollow
              : styles.AuditorphoneBox
          }
        >
          {!hasRecording ? (
            ...
            currentTicket?.phoneData?.map((item, index) => {
              if (
                item.recording !== null &&
                durations[item.recording] > 0 &&
                !uniqueRecordings.has(item.recording)
              ) {
